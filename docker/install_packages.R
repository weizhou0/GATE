...
for (pack in req_packages) {
    if(!require(pack, character.only = TRUE)) {
        ...
        install.packages(pack, repos = "https://cran.r-project.org", dependencies=TRUE)
...
req_packages <- c("R.utils", "Rcpp", "RcppParallel", "RcppArmadillo", "data.table", "RcppEigen", "Matrix", "methods", "BH", "optparse", "SPAtest", "roxygen2", "rversions","devtools", "SKAT", "RhpcBLASctl", "qlcMatrix", "dplyr", "dbplyr", "MASS")
for (pack in req_packages) {
    if(!require(pack, character.only = TRUE)) {
        install.packages(pack, repos = "https://cloud.r-project.org", dependencies=TRUE)
        print(packageVersion(pack))
